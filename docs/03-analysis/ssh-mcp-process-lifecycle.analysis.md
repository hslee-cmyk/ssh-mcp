...
#### L3 (§8.4, 수동) — 미수행, Gap 아님

- [ ] cloud0 반복 연결/해제 → `ps` 프로세스 수 상한 확인
- [ ] bg job 미폴링 방치 → 좀비(`<defunct>`) 미잔존 확인
- [ ] `SSH_MCP_IDLE_TIMEOUT_SEC=60` → 60초+tick 내 self-exit 확인
- [ ] **(신규, Gap E)** 프로세스 그룹 kill — `ssh_bg_run`(`sleep 60`) → `ssh_bg_kill` → `ps -ef | grep sleep`로 자식까지 종료됐는지 확인
- [ ] **(신규, Gap A 잔여)** `ssh_bg_run`(`bash -c "trap '' TERM; sleep 60"`) → `ssh_bg_kill` → ~5초 후 `SIGKILL, forced` 확인

Design §8.1에 "Phase: Check"로 명시된 항목이므로 Do 단계 gap이 아니라 Check 단계에서 사용자가 cloud0 접속해 수동 진행할 항목. 위 2개 신규 항목은 로컬 Windows에서 원천적으로 검증 불가능함을 실측으로 확인(§Gaps Gap E, Gap A 처리 결과 참조) — cloud0에서 반드시 확인 필요.
...
│  Functional Match Rate:   100% (Gap E 수정 후)│
│  Contract Match Rate:     100%               │
│  L1 Runtime (proxy):      100%               │
│  L2 Runtime (proxy):      100%               │
│  L3 Runtime:            미수행(수동 항목, 미산입) │
│  ─────────────────────────────────────────── │
│  Overall Match Rate:     ~100%               │
│  = (Structural × 0.2) + (Functional × 0.4)  │
│    + (Contract × 0.4)   [정적 전용 공식 —     │
│    서버 미기동, L3는 Check 수동 항목으로 별도]  │
├─────────────────────────────────────────────┤
│  ✅ Match:            전 항목 (A/B/C/D/E Fixed)│
│  ⚠️ L3 미수행:         5건 (cloud0 수동, 아래) │
│  ❌ Not implemented:   0건                    │
└─────────────────────────────────────────────┘
```

> Gap 발견 당시(gap-detector 정적 분석) 스냅샷은 Overall ~99%였다. 이후 실측(process-group kill 버그, Gap E)까지 포함해 전부 수정 완료한 최종 상태가 위 표다.
...
**처리**: ✅ **Fixed** — `ssh_bg_run`에 `start_new_session=True` 추가로 job마다 독립 프로세스 그룹 생성, `_kill_group()` 헬퍼(`os.killpg` 사용, POSIX 전용)로 `_terminate_sync`/`_kill_pid_async` 통일. Design §4.1.1(신규)에 상세 기록. **로컬 Windows에서는 그룹-kill 자체를 검증할 수 없음**(MSYS2의 자체 PID 네임스페이스가 네이티브 Windows PID와 불일치 — 실측 확인) — cloud0 L3 수동 스모크(Design §8.4 #4)로 이관.
...
**모든 Gap(A/B/C/D/E) 수정 완료, 테스트 26→29개 전부 PASS.** 남은 것은 cloud0 실배포가 필요한 L3 수동 항목 5건뿐 — Design §8.1에 이미 "Phase: Check, 수동"으로 정의된 것들이라 코드/문서 Gap이 아니다.

---

## 9. Recommended Actions

### 9.1 완료됨 (Checkpoint 5 "지금 모두 수정")

| 항목 | 파일 | 결과 |
|---|---|---|
| Gap A: SIGKILL 승격 테스트 추가 | `tests/test_lifecycle.py` | ✅ mock 기반 제어흐름 테스트 추가(실제 trap 검증은 L3로 이관) |
...
| **Gap E(신규, 실측 발견)**: process-group kill 버그 수정 | `ssh_agent.py`(`_kill_group`, `start_new_session=True`) | ✅ 완료, Design §4.1.1 신규 반영 |

### 9.2 Check 단계 수동 항목 (cloud0 필요, Gap 아님)

- [ ] 반복 연결/해제 → 프로세스 수 상한 확인
- [ ] bg job 미폴링 방치 → 좀비 미잔존 확인
- [ ] idle timeout 실동작(`SSH_MCP_IDLE_TIMEOUT_SEC` 단축) 확인
- [ ] **(신규)** process-group kill — 자식까지 종료되는지 `ps -ef` 확인
- [ ] **(신규)** SIGTERM 무시 job → 실제 SIGKILL 승격 확인(`trap '' TERM`)
...
- [x] Gap A/B/C/D 수정 완료
- [x] Gap E(신규 발견) 수정 완료
- [ ] Check 단계 수동 항목(cloud0, 5건) 진행 — 사용자 승인 후 수동 진행(Plan §5 배포 리스크)
